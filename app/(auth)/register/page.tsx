...
export default function RegisterPage() {
  ...
  const [loadingState, setLoadingState] = React.useState<"google" | "apple" | "email" | null>(null);
  ...
  const handleOAuth = async (provider: "google" | "apple") => {
    ...
    addToast(`Connecting to ${providerName}... Please select your account`, "info");
    ...
    try {
      if (supabase) {
        ...
        setTimeout(() => {
          addToast(`Registered via ${providerName} demo mode!`, "success");
          router.push("/dashboard");
        }, 800);
      }
    } catch (err: any) {
      setError(err.message || `Failed to sign up with ${providerName}.`);
      addToast(`Registration failed: ${err.message || 'Error'}`, "error");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    addToast("Creating your Invox account...", "info");
    ...
    try {
      ...
      addToast("Account created successfully! Welcome to Invox", "success");
      setTimeout(() => {
        router.push("/dashboard");
      }, 500);
    } catch (err: any) {
