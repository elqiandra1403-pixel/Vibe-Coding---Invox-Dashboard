...
export default function LoginPage() {
  ...
  const handleOAuth = async (provider: "google" | "apple") => {
    ...
    try {
      if (supabase) {
        ...
        setTimeout(() => {
          addToast(`Signed in via ${providerName} demo mode`, "success");
          router.push("/dashboard");
        }, 800);
      }
    } catch (err: any) {
      setError(err.message || `Failed to sign in with ${providerName}.`);
      addToast(`Authentication failed: ${err.message || 'Error'}`, "error");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    addToast("Authenticating your session...", "info");
    ...
    try {
      ...
      addToast("Welcome back to Invox!", "success");
      setTimeout(() => {
        router.push("/dashboard");
      }, 500);
    } catch (err: any) {
